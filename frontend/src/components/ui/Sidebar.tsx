...
import { Home, Users, BookOpen, FileText, Settings, LogOut } from "lucide-react"
...
export function Sidebar() {
  ...
  const menuItems = [
    { icon: <Home className="w-5 h-5" />, label: "Asosiy", href: "/dashboard" },
    { icon: <Users className="w-5 h-5" />, label: "Guruhlar", href: "/groups" },
    { icon: <BookOpen className="w-5 h-5" />, label: "Kurslar", href: "/courses" },
    { icon: <FileText className="w-5 h-5" />, label: "Vazifalar", href: "/assignments" },
  ];
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-100 h-screen sticky top-0 left-0">
      <div className="h-16 flex items-center px-6 border-b border-slate-50">
        <h1 className="text-xl font-bold text-mentor-primary tracking-tight">Mentor LMS</h1>
      </div>
